...
layout(location = 4) in vec4  a_TexOffset;
...
void main()
{
	vec2 texCoords = v_TexCoord * v_TexOffset.zw;
	texCoords += v_TexOffset.xy;
	//color = vec4(2-v_TexSlot, v_TexSlot-1, v_TexSlot-32, 1);
	//color = vec4(v_TexCoord,0,1) * v_Color;
	color = texture(u_Textures[int(v_TexSlot)], texCoords) * v_Color;
